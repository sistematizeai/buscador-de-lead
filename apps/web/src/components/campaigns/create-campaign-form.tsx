...
export function CreateCampaignForm() {
  ...
  const fieldClass = "h-11 rounded-xl border-slate-200 bg-white shadow-sm shadow-slate-950/[0.02] focus-visible:ring-purple-500";
      ...
      <div className="space-y-5">
          ...
          <CardContent className="space-y-5 pt-5">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <button
                  type="button"
                  onClick={() => setForm((prev) => ({ ...prev, source: "google_maps" }))}
                ...
                >
                  <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${form.source === "google_maps" ? "bg-purple-100 text-purple-600" : "bg-slate-100 text-slate-500"}`}>
                    <MapPin className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold">Google Maps</p>
                    <p className="text-[10px] text-slate-400">Negócios locais</p>
                  </div>
                </button>
                ...
                >
                  <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${form.source === "instagram" ? "bg-pink-100 text-pink-600" : "bg-slate-100 text-slate-500"}`}>
                    <Instagram className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold">Instagram</p>
                    <p className="text-[10px] text-slate-400">Perfis de marca</p>
                  </div>
                </button>

                <button
                  type="button"
                  onClick={() => setForm((prev) => ({ ...prev, source: "facebook_marketplace" }))}
                  className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${
                    form.source === "facebook_marketplace"
                      ? "border-blue-500 bg-blue-50/30 text-blue-950 ring-1 ring-blue-500"
                      : "border-slate-200 bg-white text-slate-700 hover:border-slate-300"
                  }`}
                >
                  <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${form.source === "facebook_marketplace" ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-500"}`}>
                    <Store className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold">Marketplace</p>
                    <p className="text-[10px] text-slate-400">Anúncios locais</p>
                  </div>
                </button>
              </div>
            </div>
        ...
        <Card className="border-purple-200 bg-gradient-to-br from-white to-purple-50/40 shadow-sm shadow-purple-950/[0.03]">
          ...
          <CardContent className="space-y-4 pt-5">
            <button
              type="button"
              className="flex w-full items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-colors hover:border-purple-200 hover:bg-purple-50/30"
          ...
          <CardContent className="space-y-4 pt-5">
            <div className="grid gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3">
              {queryPreview.map((query) => (
                <div key={query} className="flex items-start gap-2 rounded-xl bg-white px-3 py-2 text-sm text-slate-700 shadow-sm shadow-slate-950/[0.02]">
        ...
        <Card className="border-slate-200 bg-white shadow-sm shadow-slate-950/[0.03]">
          <CardHeader className="border-b border-slate-100">
            <CardTitle className="text-base">Oferta e abordagem</CardTitle>
          </CardHeader>
          <CardContent className="space-y-5 pt-5">
            <div className="space-y-2">
              <Label htmlFor="service">Seu serviço ou produto</Label>
              <Textarea id="service" placeholder="Descreva o que você quer vender para esses leads..." value={form.yourService} onChange={set("yourService")} className="min-h-28 resize-none rounded-xl border-slate-200 shadow-sm shadow-slate-950/[0.02] focus-visible:ring-purple-500" required />
            </div>
          ...
          <CardFooter className="flex-col items-stretch gap-3 border-t border-slate-100 sm:flex-row sm:items-center">
            {submitError && (
            ...
            <div className="flex gap-3 sm:ml-auto">
              <Button type="button" variant="outline" onClick={() => router.back()} className="rounded-xl">Cancelar</Button>
              <Button type="submit" className="rounded-xl bg-purple-600 shadow-sm shadow-purple-500/20 hover:bg-purple-700" disabled={loading}>
        ...
        <Card className="border-slate-200 bg-slate-950 text-white shadow-xl shadow-slate-950/10">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-purple-500/20 text-purple-200 ring-1 ring-purple-400/20">
                <Sparkles className="h-5 w-5" />
              </div>
              <div>
                <p className="font-semibold">Assistente da campanha</p>
                <p className="text-xs text-slate-400">Resumo dos parâmetros ativos</p>
              </div>
            </div>
            <div className="space-y-3 rounded-2xl bg-white/[0.04] p-4 text-sm">
              <div>
            ...
            <div className="rounded-2xl border border-purple-400/20 bg-purple-500/10 p-4 text-sm leading-6 text-purple-100">
...
function RegionDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto border-slate-200 bg-white p-0 text-slate-950 shadow-2xl shadow-slate-950/25">
        <DialogHeader className="flex-row items-start gap-5 space-y-0 px-8 pb-5 pt-8 text-left sm:text-left">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-white text-purple-600 shadow-sm">
          ...
          <div className="pr-10">
            ...
            <DialogDescription className="mt-2 max-w-2xl text-base leading-7 text-slate-500">
              Monte a região no formato País &gt; Estado &gt; Cidade &gt; Bairro &gt; Rua para buscar com mais precisão.
            </DialogDescription>
          </div>
        </DialogHeader>
        <div className="grid gap-6 px-8 pb-7">
          ...
          <div className="flex items-center gap-4 rounded-2xl border border-purple-200 bg-purple-50/70 p-4 shadow-sm shadow-purple-950/[0.03]">
        ...
        <DialogFooter className="border-t border-slate-200 px-8 py-6 sm:justify-end sm:space-x-4">
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="h-14 min-w-36 rounded-xl border-slate-200 bg-white px-8 text-base font-medium text-slate-700 shadow-sm hover:bg-slate-50 hover:text-slate-950">
            Cancelar
          </Button>
          <Button type="button" className="h-14 min-w-56 rounded-xl bg-purple-700 px-8 text-base font-semibold text-white shadow-lg shadow-purple-700/25 hover:bg-purple-800" onClick={() => onOpenChange(false)}>
